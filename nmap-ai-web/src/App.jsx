...
//     try {
//       const response = await axios.post('http://localhost:8000/chat', { intent, target });
...
//     } catch (error) {
//       setMessages(prev => [...prev, { type: 'error', text: 'CONNECTION_INTERRUPTED: API_OFFLINE' }]);
...
//   return (
//     <div className="min-h-screen bg-[#020617] text-slate-300 font-sans selection:bg-blue-500/30 overflow-hidden flex flex-col">
...
//               All generated commands are validated against the Knowledge Graph before execution.
//             </p>
//           </div>
//         </aside>
...
//                 <p className="font-mono text-sm uppercase tracking-widest">Waiting for Operator Input...</p>
//               </div>
//             )}
...
//                           {msg.error || "The request is not related to Nmap or network scanning."}
//                         </p>
//                       </div>
//                     ) : (
...
//                           </div>
//                           <div className="flex items-center gap-2 text-[10px] font-mono bg-slate-950 p-2 rounded border border-slate-800">
//                             <Zap size={12} className={msg.is_functional ? "text-yellow-500" : "text-slate-500"} />
//                             FUNCTIONAL: {msg.is_functional ? 'VERIFIED' : 'PENDING'}
//                           </div>
//                         </div>
//                       </div>
//                     )}
...
//                 <input
//                   className="w-full bg-slate-900 border border-slate-800 rounded-lg py-3 pl-10 pr-4 text-sm focus:outline-none focus:border-blue-500 transition-all font-mono"
...
//                   <input
//                     className="w-full bg-slate-900 border border-slate-800 rounded-lg py-3 pl-10 pr-4 text-sm focus:outline-none focus:border-blue-500 transition-all"
...
function App() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { type: 'bot', ...response.data, timestamp: new Date().toLocaleTimeString() }]);
      setSystemStatus('READY');
    } catch (error) {
          ...
          <div className="p-6 bg-slate-950/90 backdrop-blur-xl border-t border-blue-900/30">
            <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="md:col-span-1 relative">
                <Globe className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" size={16} />
                <input
                  className="w-full bg-slate-900 border border-slate-800 rounded-lg py-3 pl-10 pr-4 text-sm focus:outline-none focus:border-blue-500 transition-all font-mono text-white"
                  placeholder="TARGET_IP"
                  value={target}
                  onChange={(e) => setTarget(e.target.value)}
                />
              ...
              <div className="md:col-span-3 flex gap-3">
                <div className="flex-1 relative">
                  <Terminal className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" size={16} />
                  <input
                    className="w-full bg-slate-900 border border-slate-800 rounded-lg py-3 pl-10 pr-4 text-sm focus:outline-none focus:border-blue-500 transition-all text-white"
                    placeholder="ENTER_SCAN_INTENT..."
                    value={intent}
                    onChange={(e) => setIntent(e.target.value)}
                    onKeyPress={(e) => e.key === 'Enter' && handleSend()}
